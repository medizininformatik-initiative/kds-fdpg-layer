...
| `identifier` | Identifikation | Identifikator | Ein Identifikator für die Medikationsverabreichung |
...
| `medication[x]` | Medikation[x] | Medikation | Medikation, die verarbreicht wurde. Code oder Referenz auf Medication-Objekt. |
| `medication[x]:medicationReference` | Medikation[x] | What was administered | Identifies the medication that was administered. This is either a link to a resource representing the details of the medication or a simple attribute carrying a code that identifies the medication ... |
| `medication[x]:medicationCodeableConcept` | Medikation[x] | What was administered | Identifies the medication that was administered. This is either a link to a resource representing the details of the medication or a simple attribute carrying a code that identifies the medication ... |
| `subject` | BezugZuPatient | Who received medication | The person or animal or group receiving the medication. |
| `context` | BezugZuFall | Kontext | Fall oder Kontakt, bei dem die Medikation verabreicht wurde. |
| `effective[x]` | Verabreichungszeitpunkt[x] | Zeitpunkt oder Zeitraum | Zeitpunkt oder Zeitraum in dem die Medikation verabreicht wurde. |
| `effective[x]:effectiveDateTime` | Verabreichungszeitpunkt[x] | Zeitpunkt | Zeitpunkt, zu der die Medikation verabreicht wurde. |
| `effective[x]:effectivePeriod` | Verabreichungszeitpunkt[x] | Zeitraum | Zeitraum, in dem die Medikation verabreicht wurde. |
| `performer` | Durchfuehrender | Ausführende\*r | Wer die Medikation verabreicht hat und was sie getan haben. |
| `reasonCode` | Behandlungsgrund[x] | Grund Code | Grund für die Medikationverabreichung als Code. |
| `reasonReference` |  | Grund Referenz | Grund für die Medikationsverabreichung als Referenz auf Condition- oder Observation-Objekt. |
| `request` | BezugZuVerordnung | Verordnung | Verordnung zur Medikationsverabreichung. |
| `note` | Hinweis | Hinweis | Zusätzliche Informationen zur Medikationsverabreichung als Freitext. |
...
| `meta.source` |  | Identifies where the resource comes from | A uri that identifies the source system of the resource. This provides a minimal amount of [Provenance](provenance.html#) information that can be used to track or differentiate the source of inform... |
| `meta.profile` |  | Profiles this resource claims to conform to | A list of profiles (references to [StructureDefinition](structuredefinition.html#) resources) that this resource claims to conform to. The URL is a reference to [StructureDefinition.url](structured... |
| `identifier` | Identifikation | Identifikator | Ein Identifikator für die Medikationsverordnung |
| `status` | Status | Status | aktiv \| pausiert \| abgebrochen \| abgeschlossen \| Eingabe fehlerhaft \| abgebrochen \| Entwurf \| unbekannt |
| `intent` | Verordnungsdetails | Intention | Vorschlag \| Plan \| Auftrag \| Original-Auftrag \| ergänzender Auftrag \| Erfüllungsauftrag \| Vorgangsauftrag \| Option |
| `medication[x]` | Medikation[x] | Medikation | Medikation, die verordnet wurde. Code oder Referenz auf Medication-Objekt. |
| `medication[x]:medicationReference` | Medikation[x] | Medication to be taken | Identifies the medication being requested. This is a link to a resource that represents the medication which may be the details of the medication or simply an attribute carrying a code that identif... |
| `medication[x]:medicationCodeableConcept` | Medikation[x] | Medication to be taken | Identifies the medication being requested. This is a link to a resource that represents the medication which may be the details of the medication or simply an attribute carrying a code that identif... |
| `subject` | BezugZumPatient | Who or group medication request is for | A link to a resource representing the person or set of individuals to whom the medication will be given. |
| `encounter` | BezugZumFall | Fall / Kontakt | Fall oder Kontakt, bei dem die Medikation verordnet wurde. |
| `authoredOn` | DatumDerVerordnung | Datum der Verordnung | Das Datum, an dem die Verordnung ursprünglich verfasst wurde. |
| `requester` | AutorInformantDerVerordnung | Anforderer | Die Person, Organisation oder das Gerät, die die Verordnung initiiert hat und für deren Aktivierung verantwortlich ist. |
| `reasonCode` | Behandlungsgrund[x] | Grund Code | Grund für die Medikationverordnung als Code. |
...
| `note` | Hinweis | Hinweis | Zusätzliche Informationen zur Medikationsverordnung als Freitext. |
| `dosageInstruction` | Dosierungsinstruktionen | Dosierungsanweisung | Gibt an, wie das Medikament vom Patienten zu verwenden ist. |
| `substitution` | Ersatzmedikation[x] | Substitution | Etwaige Einschränkungen bei der Substitution von Medikamenten |
| `priorPrescription` | VorherigeMedikationsverordnungen | Vorherige Verschreibung | Eine Verschreibung, die ersetzt wird |
...
| `meta.source` |  | Identifies where the resource comes from | A uri that identifies the source system of the resource. This provides a minimal amount of [Provenance](provenance.html#) information that can be used to track or differentiate the source of inform... |
| `meta.profile` |  | Profiles this resource claims to conform to | A list of profiles (references to [StructureDefinition](structuredefinition.html#) resources) that this resource claims to conform to. The URL is a reference to [StructureDefinition.url](structured... |
| `identifier` | Identifikation | Identifikator | Ein Identifikator für den Medikationseintrag |
| `basedOn` | BezugZuVerordnung, BezugZuAbgabe | Basiert auf | Ein Plan oder eine Anforderung, die durch den Medikationseintrag erfüllt wird. |
| `partOf` |  | Teil von | Ein größeres Ereignis, von dem dieses spezielle Ereignis ein Bestandteil oder Schritt ist. |
| `status` | Status | Status | aktiv \| abgeschlossen \| Eingabe fehlerhaft \| intendiert \| gestoppt \| pausiert \| unbekannt\| nicht eingenommen |
| `category` |  | Kategorie | Eine Kategorie, die dem Medikationseintrag zugeordnet ist. |
| `medication[x]` | Medikation[x] | Medikation | Medikation, welche Gegenstand des Eintrags ist. Code oder Referenz auf Medication-Objekt. |
| `medication[x]:medicationReference` | Medikation[x] | Medikation Referenz | Referenz auf eine Medication-Ressource. |
| `medication[x]:medicationCodeableConcept` | Medikation[x] | Medikation Code | Code für das Medikament, welches Gegenstand des Eintrags ist. |
| `subject` | BezugZumPatient | Who is/was taking the medication | The person, animal or group who is/was taking the medication. |
| `context` | BezugZumFall | Kontext | Fall oder Kontakt in Verbindung mit dem Medikationseintrag |
| `effective[x]` | Einnahmedauer | Zeitpunkt oder Zeitraum | Die Zeitspanne, in der angegeben wird, dass der Patient das Medikament eingenommen hat oder einnehmen wird. |
| `effective[x]:effectiveDateTime` | Einnahmedauer | Zeitpunkt | Der Zeitpunkt, an dem der Patient das Medikament eingenommen hat oder einnehmen wird. |
| `effective[x]:effectivePeriod` | Einnahmedauer | Zeitraum | Der Zeitraum, in dem der Patient das Medikament eingenommen hat oder einnehmen wird. |
| `dateAsserted` | DatumDesEintrags | Datum Bestätigung | Das Datum, an dem der Medikationseintrag von der Informationsquelle bestätigt wurde. |
| `informationSource` | AutorInformantDesEintrags | Informationsquelle | Die Person oder Organisation, die die Information über die Einnahme dieses Medikaments bereitgestellt hat. |
| `reasonCode` | Behandlungsgrund[x] | Grund Code | Grund für den Medikationseintrag als Code. |
| `reasonReference` |  | Grund Referenz | Grund für den Medikationseintrag als Referenz auf Condition- oder Observation-Ressource. |
| `note` | Hinweis | Hinweis | Zusätzliche Informationen zum Medikationseintrag als Freitext. |
| `dosage` | Dosierung | Dosierung | Details zur Dosierung des Medikamentes. |
...
| `meta.source` |  | Identifies where the resource comes from | A uri that identifies the source system of the resource. This provides a minimal amount of [Provenance](provenance.html#) information that can be used to track or differentiate the source of inform... |
| `meta.profile` |  | Profiles this resource claims to conform to | A list of profiles (references to [StructureDefinition](structuredefinition.html#) resources) that this resource claims to conform to. The URL is a reference to [StructureDefinition.url](structured... |
| `code` | Code | Code | Code für Medikation |
| `form` | Darreichungsform | Darreichungsform | Darreichungsform des Medikaments |
| `ingredient` | Bestandteil | Bestandteil | Ein Bestandteil des Medikaments |
...
| `meta.source` |  | Identifies where the resource comes from | A uri that identifies the source system of the resource. This provides a minimal amount of [Provenance](provenance.html#) information that can be used to track or differentiate the source of inform... |
| `meta.profile` |  | Profiles this resource claims to conform to | A list of profiles (references to [StructureDefinition](structuredefinition.html#) resources) that this resource claims to conform to. The URL is a reference to [StructureDefinition.url](structured... |
| `identifier` | Identifikation | Business identifier | Identifier for the List assigned for business purposes outside the context of FHIR. |
| `status` | Status | Status | aktuell \| inaktiv \| Eingabe fehlerhaft |
| `mode` | Modus | Modus | Arbeitsmodus \| Momentaufnahme \| Änderungen |
| `code` | Typ | Code | Enthält Codes für Medikationsliste und Kontext der Medikationsliste. |
| `subject` | BezugZumPatient | If all resources have the same subject | The common subject (or patient) of the resources that are in the list if there is one. |
| `encounter` | BezugZumFall | Fall oder Kontakt | Fall oder Kontakt, bei dem die Medikationsliste erstellt wurde. |
| `date` |  | Datum | Das Datum, an dem die Medikationsliste erstellt wurde. |
| `entry` | Medikationseintrag | Eintrag | Eintrag in der Medikationsliste vom Typ MedicationStatement. |
