...
| `identifier` | Identifikation | Identifikator | Kennung/en, unter der/denen diese Laboranforderung bekannt ist. |
| `identifier:anforderung` | Identifikation | Anforderungs-Identifikator | Der eindeutige Identifikator der Laboranforderung. |
...
| `code` | Laborparameter | Code | Ein LOINC Code für den Laborparameter bzw. Labortest, der beauftragt wurde. |
...
| `authoredOn` | Anforderungsdatum | Anforderungsdatum | Das Datum und ggfs. Uhrzeit, an dem der Laborauftrag ursprünglich verfasst wurde. |
| `specimen` | Probenmaterial | Probenmaterial | Eine oder mehrere Bioproben, die der Labortest verwendet. |
...
| `meta.source` |  | Identifies where the resource comes from | A uri that identifies the source system of the resource. This provides a minimal amount of [Provenance](provenance.html#) information that can be used to track or differentiate the source of inform... |
| `meta.profile` |  | Profiles this resource claims to conform to | A list of profiles (references to [StructureDefinition](structuredefinition.html#) resources) that this resource claims to conform to. The URL is a reference to [StructureDefinition.url](structured... |
| `identifier` | Identifikation | Identifikator | Eindeutige Identifikatoren, unter denen dieser Laborbefund geführt wird. |
| `identifier:befund` | Identifikation | Filler-Identifikator | Identifikator, der vom Laborinformationssystem (Filler) vergeben wurde. |
| `basedOn` |  | Basiert auf | Bezug zum Laborauftrag, auf dem dieser Laborbefund basiert. |
| `status` | Status | Status | registriert \| teilweise \| vorläufig \| final |
...
| `effective[x]` | KlinischerBezugszeitpunkt | Klinisches Bezugsdatum | Zeitpunkt, zu dem die gemessene Eigenschaft im Probenmaterial e.g. Analytkonzentration mutmaßlich der Eigenschaft im Patienten entsprach. |
| `issued` | Dokumentationsdatum | Dokumentationsdatum | Zeitpunkt, an dem der Laborbefund dokumentiert wurde. |
...
| `identifier` | Identifikation | Identifikator | Kennung/en, unter der/denen diese Laboruntersuchung bekannt ist. |
| `identifier:analyseBefundCode` | Identifikation | Analyse-Befund-Code | A unique identifier assigned to this observation. |
| `status` | Status | Status | abgeschlossen |
| `category` | Bereich, Gruppe | Kategorie | Klassifikation der Laboruntersuchung im diagnostischen Fachbereich und der Laborgruppe |
| `code` | Code | Code | LOINC-Code, der den gemessenen Laborparameter bzw. durchgeführten Labortest beschreibt. |
...
| `effective[x]` | KlinischerBezugszeitpunkt | Untersuchungszeitpunkt | Klinischer Bezugszeitpunkt der Laboruntersuchung |
| `issued` | Dokumentationsdatum | Dokumentationsdatum | Zeitpunkt, an dem das Ergebnis der Laboruntersuchung dokumentiert wurde. |
| `value[x]` | Laborparameter, Messwert | Messwert | Wert der Analyse |
| `value[x]:valueQuantity` | Laborparameter, Messwert | Actual result | The information determined as a result of making the observation, if the information has a simple value. |
| `value[x]:valueCodeableConcept` | Laborparameter, Messwert | Actual result | The information determined as a result of making the observation, if the information has a simple value. |
| `value[x]:valueRange` | Laborparameter, Messwert | Actual result | The information determined as a result of making the observation, if the information has a simple value. |
| `value[x]:valueRatio` | Laborparameter, Messwert | Actual result | The information determined as a result of making the observation, if the information has a simple value. |
| `dataAbsentReason` |  | Grund für fehlende Daten | unbekannt \| maskiert \| nicht anwendbar \| Fehler \| nicht durchgeführt |
| `interpretation` | Interpretation | Interpretation | Eine kategorische Bewertung des Messwertes. Zum Beispiel hoch, niedrig, normal. |
| `note` | Kommentar | Hinweis | Zusätzliche Informationen zur Laboruntersuchung als Freitext. |
| `method` | Untersuchungsmethode | Untersuchungsmethode | Konkrete Untersuchungsmethode, wenn der verwendete LOINC-Code für den Laborparameter keine Methode enthält. |
| `specimen` | Probenmaterial | Probenmaterial | Probe, auf deren Basis die Laboruntersuchungen angefertigt werden |
| `device` |  | Gerät | Gerät, das zur Generierung der Messwerte verwendet wurde. |
| `referenceRange` | Referenzbereich | Referenzbereich | Bereich, in dem der Messwert als normal oder empfohlen betrachtet wird. |
