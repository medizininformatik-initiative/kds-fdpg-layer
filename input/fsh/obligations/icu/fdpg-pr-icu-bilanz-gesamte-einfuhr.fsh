Profile: FDPG_PR_ICU_Bilanz_Gesamte_Einfuhr
Parent: MII_PR_ICU_Bilanz_Gesamte_Einfuhr
Id: fdpg-pr-icu-bilanz-gesamte-einfuhr
Title: "FDPG PR ICU Bilanz Gesamte Einfuhr"
Description: "FDPG Profil - MII PR ICU Bilanz Gesamte Einfuhr"
* insert FDPGMetadata
* insert Translation(^title, de-DE, MII PR ICU Bilanz Gesamte Einfuhr)
* insert Translation(^title, en-US, FDPG PR ICU Bilanz Gesamte Einfuhr)

// MustSupport flags
* Observation.value[x].code MS

// Obligations
// ObligationBidirectional
* insert ObligationBidirectional(Observation.value[x].code)

