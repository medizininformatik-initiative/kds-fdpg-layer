Profile: FDPG_PR_ICU_Bilanz_Blutverlust
Parent: MII_PR_ICU_Bilanz_Blutverlust
Id: fdpg-pr-icu-bilanz-blutverlust
Title: "FDPG PR ICU Bilanz Blutverlust"
Description: "FDPG Profil - MII PR ICU Bilanz Blutverlust"
* insert FDPGMetadata
* insert Translation(^title, de-DE, MII PR ICU Bilanz Blutverlust)
* insert Translation(^title, en-US, FDPG PR ICU Bilanz Blutverlust)

// MustSupport flags
* Observation.value[x].code MS

// Obligations
// ObligationBidirectional
* insert ObligationBidirectional(Observation.value[x].code)

